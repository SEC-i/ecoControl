...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        directory = join(
            BASE_DIR, "server", "forecasting", "simulation", "demodata")
        ...
        with open(join(directory, 'demo_weather.csv'), "rb") as file_obj:
            ...
            for row in reader:
                t = datetime.fromtimestamp(
                    int(row[t_index])).replace(tzinfo=utc)
        ...
        with open(join(directory, 'demo_weather_forecast.csv'), "rb") as file_obj:
            ...
            values = []
            for row in reader:
                time_c = datetime.fromtimestamp(
                    int(row[tc_index])).replace(tzinfo=utc)
                time_fc = datetime.fromtimestamp(
                    int(row[tf_index])).replace(tzinfo=utc)
    ...
    def flush_data(self, values, forecasts):
        ...
        for row in values:
            if forecasts:
                vals = [row[0], connection.ops.value_to_db_datetime(
                    row[1]), connection.ops.value_to_db_datetime(row[2])]
            else:
