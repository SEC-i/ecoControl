...
class ForecastTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        raw_dataset_2014 = DataLoader.load_from_file(
            path, "Strom - Verbrauchertotal (Aktuell)", "\t")
        self.dataset_2014 = StatisticalForecast.make_hourly(
    ...
    def setup_forecast(self):
        ...
        self.forecast = DayTypeForecast(
            self.env, hourly_data, 1, None, (0.0000000, 0.0, 1.0))
...
# from server.forecasting.tools import plotting
        for i in range(7):
            ...
            rmse = self.rmse(
                self.dataset_2014[:len(fc.forecasted_demands[i])], fc.forecasted_demands[i])
            self.assertTrue(rmse < 30.0, "MSE of " + str(
                rmse) + "for day" + str(i) + " is way too high")
    ...
    def test_forecast_at(self):
        self.setup_forecast()
        (at_now, week_index, hour_index) = self.forecast.forecast_at(
            self.env.now)

        self.assertTrue(
            week_index == 0, "day should be 0 but was " + str(week_index))
        self.assertTrue(
            hour_index == 0, "day should be 0 but was " + str(hour_index))
        ...
        self.assertTrue(
            week_index == 1, "week should be 1 but was " + str(week_index))
        self.assertTrue(
            hour_index == 2, "hour should be 2 but was " + str(hour_index))
    ...
    def test_append_data(self):
        ...
        self.assertSequenceEqual(self.forecast.demands[start.weekday()][
                                 -four_weeks:], split_demands14[start.weekday()][-four_weeks:])

        self.assertSequenceEqual(self.forecast.demands[3][
                                 -four_weeks:], split_demands14[3][-four_weeks:])

    def test_approximate_index(self):
        data = [1, 2, 3, 5, 6, 7, 8]
        self.assertTrue(approximate_index(data, 4) in [
                        2, 3], "index approximation was wrong")
