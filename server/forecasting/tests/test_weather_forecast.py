...
class UpdateWeatherEstimatesTest(TestCase):
    ...
    def test_crawled_data_in_db(self):
        ...
        # there should be results for 14 days
        ...
        last_target_timestamp = aware_timestamp_from_seconds(last_target_time)
        
        early_new_stamp = aware_timestamp_from_seconds(current_day-60)
        
        results = WeatherValue.objects.filter(
            timestamp__gt = early_new_stamp,
            target_time__gt = last_target_timestamp )
        self.assertTrue(results)
