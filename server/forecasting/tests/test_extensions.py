...
class ExtensionsTest(unittest.TestCase):
    ...
    def test_doubleseasonal(self):
        ...
        for a in [0.0, 0.5,1.0]:
            for b in [0.0, 0.5,1.0]:
                py_forecast,p,insample = double_seasonal(self.dataset[:-input_length], 24, 24 * 7, forecast, 
                             alpha=a, beta=0.0, gamma=a, delta=a, autocorrelation=b)
                cy_forecast,p,insample = Cdouble_seasonal(self.dataset[:-input_length], 24, 24 * 7, forecast, 
                             alpha=a, beta=0.0, gamma=a, delta=a, autocorrelation=b)
                
                #exclude very high values from testing, as these will have floating point accuracy issues
                if abs(numpy.mean(py_forecast)) < 10**9:
                    self.assertTrue(self.rmse(py_forecast, cy_forecast) < 0.5, "python and cython dshw-forecasts differ significantly.")
    ...
    def test_multiplicative(self):
        ...
        for a in [0.0, 0.5,1.0]:
            for b in [0.0, 0.5,1.0]:
                py_forecast,p,insample = multiplicative(self.dataset[:-input_length], 24, forecast, 
                             alpha=a, beta=b, gamma=b)
                cy_forecast,p,insample = Cmultiplicative(self.dataset[:-input_length], 24,  forecast, 
                             alpha=a, beta=b, gamma=b)
                
                #exclude very high values from testing, as these will have floating point accuracy issues
                if abs(numpy.mean(py_forecast)) < 10**9:
                    self.assertTrue(self.rmse(py_forecast, cy_forecast) < 0.5, "python and cython multiplicative-forecasts differ significantly.")
    ...
    def test_performance(self):
        ...
        t2 = time.time()
        for i in range(100):
            Cdouble_seasonal(self.dataset[:-input_length], 24, 24 * 7, forecast, 
                                 alpha=0.5, beta=0.0, gamma=0.5, delta=0, autocorrelation=0.2)
        cy_timing = time.time() - t2
        #print py_timing, cy_timing
        self.assertTrue(cy_timing < py_timing, "cython version is slower than python. WTF?")
