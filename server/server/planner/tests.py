...
class TestPlanner(TestCase):
    ...
    def setUpClass(cls): 
        ...
        SensorEntry.objects.create(sensor=sensor,value = 547,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=10))
        SensorEntry.objects.create(sensor=sensor,value = 546,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=9))
        SensorEntry.objects.create(sensor=sensor,value = 545,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=8))
        SensorEntry.objects.create(sensor=sensor,value = 548,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=7))
        SensorEntry.objects.create(sensor=sensor,value = 547,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=6))
        SensorEntry.objects.create(sensor=sensor,value = 544,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=5))
        SensorEntry.objects.create(sensor=sensor,value = 543,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=4))
        SensorEntry.objects.create(sensor=sensor,value = 542,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=3))
        SensorEntry.objects.create(sensor=sensor,value = 541,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=2))
        SensorEntry.objects.create(sensor=sensor,value = 540,timestamp=datetime.now().replace(tzinfo=cet)-timedelta(seconds=1))
    ...
    def setUp(self):
        ...
        self.sensor_delta.timestamp = datetime.now().replace(tzinfo=cet)
    ...
    def testDelta(self):
        ...
        entry.value = 600
        entry.timestamp = datetime.now().replace(tzinfo=cet)
        entry.save()
        ...
        entry2.value = 700
        entry2.timestamp = datetime.now().replace(tzinfo=cet)
        entry2.save()
    ...
    def testTaskCreation(self):
        ...
        self.assertTrue(tasks[0].execution_timestamp > datetime.now().replace(tzinfo=cet))
